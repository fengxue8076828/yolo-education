...
const PriceBox = ({price,lang}:{price:string,lang:string}) => {
  ...
  return (
    <div className='bg-white self-start w-full lg:w-[20%] px-7 py-5 gap-5 md:px-7 md:py-10 flex flex-col md:gap-10 rounded order-first lg:order-last'>
      ...
      <div className='flex flex-col gap-3 md:gap-7 md:flex-row lg:flex-col'>
        ...
        <button className='w-full md:w-[200px] lg:w-full px-3 py-3 bg-dark-blue text-white rounded-lg' onClick={()=>{router.push("#register")}}>{lang==="hu"?"Regisztráció":lang==="en"?"Registration":"注册课程"}</button>
      </div>    
    </div>
  )
}
